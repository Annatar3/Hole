...
document.getElementById('btn-dev-delete').addEventListener('click', function() {
  ...
  if (!confirm('Remove "' + name + '" from the local registry? This does not affect the remote machine.')) return
...
function loadFleet() {
  ...
  }).catch(function() { /* server restarting — silently skip */ })
}
...
function renderFleet() {
  ...
  el.innerHTML = devs.map(function(d) {
    ...
    return '<div class="device-item' + active + '" role="button" tabindex="0" data-name="' + esc(n) + '">' +
  ...
  el.querySelectorAll('.device-item').forEach(function(el) {
    ...
    el.addEventListener('keydown', function(e) {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectDevice(this.dataset.name) }
    })
  })
}
        ...
        .then(function(data) {
          ...
          if (data.error) {
            ...
            if (msg.indexOf('Service "') === 0 && msg.indexOf('not found on device') !== -1) {
              msg += ' Add this service key to the device registry first (or run hole up --forward and register the printed service key).'
            }
...
document.getElementById('btn-exec-run').addEventListener('click', function() {
  ...
  fetch('/api/exec', {
    ...
    .then(function(data) {
      ...
      if (data.error) { outEl.innerHTML += '<span style="color:var(--red)">Error: ' + esc(data.error) + '</span>'; return }
      var html = ''
