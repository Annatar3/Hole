...
// ---------------------------------------------------------------------------
function resolveTarget (target, service) {
...
export async function openProxy ({ target, service = null, port = 2222, relay }) {
  ...
  const proxy = net.createServer(async (localConn) => {
    ...
    try {
      ...
      const hints = {
        PEER_NOT_FOUND:    '→ Is the agent running on the remote host and announced on DHT?',
        HOLEPUNCH_ABORTED: '→ NAT traversal failed. Both sides may be behind symmetric NAT. Use --relay <host:port>',
        HOLEPUNCH_TIMEOUT: '→ Hole punch timed out. Check if outbound UDP is allowed or try relay mode.',
        ETIMEDOUT:         '→ Network timeout. Check your internet connection.',
        ECONNREFUSED:      '→ Connection refused by the remote DHT node.'
      }
  ...
  await new Promise(resolve => proxy.listen(localPort, '127.0.0.1', resolve))
...
export async function exec ({ target, user, service = null, relay, identity = null, cmd = [] }) {
  ...
  args.push(`${loginUser}@localhost`, ...cmd)
...
export async function copy ({ target, src, dest, user, relay, identity = null }) {
  ...
  // Determine which side is remote; rewrite device:/path → user@localhost:/path
  const REMOTE_RE = /^[^/:\\]+:(.+)$/
  ...
  const remoteSpec = `${loginUser}@localhost:${remotePath}`
...
//
//   hole ssh my-remote                       → ssh <$USER>@localhost
//   hole ssh my-remote admin                 → ssh admin@localhost
...
export async function ssh ({ target, user, service = null, relay, identity = null, sshArgs = [] }) {
  ...
  cmd.push(`${loginUser}@localhost`, ...sshArgs)
  ...
  const child = spawn('ssh', cmd, { stdio: 'inherit' })
