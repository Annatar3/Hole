  ...
  const proxy = net.createServer(async (localConn) => {
    ...
    try {
      ...
      const hints = {
        ...
        HOLEPUNCH_ABORTED: '→ Both sides may be behind strict NAT. Try --relay <host:port>',
...
// ---------------------------------------------------------------------------
export async function exec ({ target, user, service = null, relay, cmd = [] }) {
  if (!cmd.length) {
...
// ---------------------------------------------------------------------------
export async function copy ({ target, src, dest, user, relay }) {
  const loginUser = user || process.env.USER || process.env.USERNAME || 'user'
...
// ---------------------------------------------------------------------------
export async function ssh ({ target, user, service = null, relay, sshArgs = [] }) {
  const { localPort, hexKey, close } = await openProxy({ target, service, port: 0, relay })
