...
import { log, warn, CONNECT_TIMEOUT_MS, hintForConnectionStatus } from './utils.js'
...
export async function ping ({ target, count = 4, relay }) {
  ...
  if (mean !== null) {
    ...
    if (loss > 0) {
      ...
      const hint = firstStatus ? hintForConnectionStatus(firstStatus, relay) : null
      if (hint) console.log(`Hint: Intermittent failures detected. ${hint}`)
    }
  } else {
    const firstStatus = results.find(r => r.status !== 'ok')?.status
    const hint = firstStatus ? hintForConnectionStatus(firstStatus, relay) : null
