...
function systemdUnit (execStart) {
  return `[Unit]
Description=Hole Agent — P2P SSH tunnel via HyperDHT
...
function linuxInstall (agentArgs) {
  ...
  const unit   = systemdUnit(`${exec} agent${args}`)
...
function windowsInstall (agentArgs) {
  ...
  const args = ['agent', ...agentArgs].join(' ')

  // schtasks /create /tn "Hole Agent" /tr "C:\...\hole.exe agent --name x" /sc ONLOGON /f
  const cmd = [
    'schtasks /create',
    `/tn "Hole Agent"`,
    `/tr "\\"${exec}\\" ${args}"`,
...
export function install (agentArgs = []) {
  ...
  die(`Unsupported platform: ${process.platform}. Supported: linux, win32`)
}
